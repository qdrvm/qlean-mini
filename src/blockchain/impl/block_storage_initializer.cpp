...
namespace lean::blockchain {
  ...
  BlockStorageInitializer::BlockStorageInitializer(
    ...
    // Try to get anchor block header from storage
    ...
    if (anchor_block_existing_res.has_error()) {
      block_storage.logger_->critical(
          "Database error at check existing anchor block: {}",
    ...
    if (not anchor_header_is_exist) {
      ...
      if (res.has_error()) {
        block_storage.logger_->critical(
            "Database error at store anchor block into: {}", res.error());
      ...
      if (assignment_res.has_error()) {
        block_storage.logger_->critical(
            "Database error at assigning anchor block hash: {}",
      ...
      if (sel_leaves_res.has_error()) {
        block_storage.logger_->critical(
            "Database error at set anchor block as leaf: {}",
      ...
      if (state_res.has_error()) {
        block_storage.logger_->critical(
            "Database error at store anchor state into: {}", state_res.error());
